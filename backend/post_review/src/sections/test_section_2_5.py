...
Test script for Section 2_5 - Same rules as Section 2_1
...
class Section2_5Tester:
    def __init__(self, pdf_path=None):
        self.section_name = "Section_2_5"
        
        # Make test directory relative to the project root
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        self.test_dir = os.path.join(project_root, "data", "test_sections", "section_2_5_test")
    ...
    def load_extracted_image(self) -> str:
        ...
        try:
            ...
            image_filename = f"section_2_5_extracted.png"
    ...
    def analyze_with_gpt4o(self, image_path: str) -> dict:
        ...
        try:
            ...
            # Get analysis prompt
            prompt = self.get_section_2_5_analysis_prompt()
    ...
    def apply_changes_to_document(self, analysis_result: dict) -> bool:
        ...
        try:
            ...
            # Apply Section 2_5 changes using the reference implementation
            from section_implementations_reference import SectionImplementationsReference
            ref_impl = SectionImplementationsReference()
            changes_applied = ref_impl.apply_section_2_5_changes(doc, analysis_result)
            
            # Save processed document
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_path = f"{self.test_dir}/section_2_5_processed_{timestamp}.docx"
    ...
    def get_section_2_5_analysis_prompt(self) -> str:
        """
        Analysis prompt for Section 2_5 - SAME RULES AS SECTION 2_1
        """
        return """
        ...
        RULE 2: ROW DELETION (Critical Override)
        - If BOTH left box AND right box have ANY diagonal/cross marks (even partial marks)
        - The diagonal marks DO NOT need to go through the entire box content
        - The diagonal marks can be partial - just going through part of the text is sufficient
        - If BOTH boxes have diagonal marks detected (even if they don't cover all content)
        - Then the ENTIRE ROW should be deleted (not just individual content)
        - This rule takes PRIORITY over individual deletions
        ...
        Return detailed JSON:
        {
            ...
            "row_deletion_rule": {
                "left_box_has_diagonal_marks": true/false,
                "right_box_has_diagonal_marks": true/false,
                "both_boxes_have_marks": true/false,
                "should_delete_entire_row": true/false,
                "explanation": "reasoning for row deletion decision (row deleted if BOTH boxes have ANY diagonal marks, even partial)"
            },
            "visual_description": "comprehensive description of all deletion marks detected"
        }
        """
    
    def find_section_2_5_table_row(self, doc: Document) -> tuple:
        """Find the table and row index for Section 2_5 content"""
        # Section 2_5 should be different from all previous sections
        # Look for Section 2_5 specific keywords or patterns
        
        for table_idx, table in enumerate(doc.tables):
            if len(table.rows) >= 5 and len(table.columns) >= 2:  # At least 5 rows for multiple sections
                # Check the 5th row (index 4) for Section 2_5 content
                for row_idx in range(4, len(table.rows)):  # Start from row 4 (5th row)
                    row = table.rows[row_idx]
                    if len(row.cells) >= 2:
                        left_cell = row.cells[0].text.strip().lower()
                        right_cell = row.cells[1].text.strip().lower()
                        
                        # Look for Section 2_5 indicators (estate planning related)
                        ...
                        # Check if this row contains Section 2_5 content
        ...
        # Fallback: assume first table, fifth row (Row 4) for Section 2_5
        if len(doc.tables) > 0 and len(doc.tables[0].rows) > 4:
            return 0, 4
                ...
                for item in items_to_delete:
                    if item.get("should_delete", False):
                        ...
                        for para_idx, para in enumerate(paragraphs):
                            ...
                            # Strategy 3: Estate planning terms matching
                            estate_terms = ['estate', 'planning', 'will', 'testament', 'executor', 'beneficiary', 'inheritance', 'trust']
                            if any(term in target_text.lower() for term in estate_terms) and \
                               any(term in para_text.lower() for term in estate_terms):
    ...
    def apply_section_2_5_changes(self, doc: Document, analysis_data: dict) -> list:
        """
        Apply Section 2_5 changes - SAME LOGIC AS SECTION 2_1
        """
        changes_applied = []
        ...
        table_idx, row_idx = self.find_section_2_5_table_row(doc)
    ...
    def save_analysis_results(self, analysis_data: dict):
        ...
        try:
            analysis_file = os.path.join(self.test_dir, "section_2_5_analysis.json")
    ...
    def run_analysis_only(self, progress_callback=None):
        ...
        print(f"Starting Section_2_5 Analysis")
        ...
        print(f"📊 Step 1: Loading PDF and cutting Section_2_5...")
        section_image_path = self.load_extracted_image()
        if not section_image_path:
            print(f"❌ Section_2_5 Analysis Failed!")
            return False
        ...
        print(f"🧠 Step 2: Analyzing Section_2_5 with GPT-4o...")
        ...
        print(f"🎉 Section_2_5 Analysis Complete!")
        return True
    
    def run_test(self):
        """Run the complete Section 2_5 test"""
        print(f"🚀 Starting Section 2_5 Test (Same rules as Section 2_1)")
        ...
        analysis_path = f"{self.test_dir}/section_2_5_analysis.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="2 5 Tester")
    ...
    tester = Section2_5Tester(pdf_path=args.pdf_path)
