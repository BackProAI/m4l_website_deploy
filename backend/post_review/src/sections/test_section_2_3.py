...
Test script for Section 2_3 - Same rules as Section 2_1
...
class Section2_3Tester:
    def __init__(self, pdf_path=None):
        self.section_name = "Section_2_3"
        
        # Make test directory relative to the project root
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        self.test_dir = os.path.join(project_root, "data", "test_sections", "section_2_3_test")
    ...
    def load_extracted_image(self) -> str:
        ...
        try:
            ...
            image_filename = f"section_2_3_extracted.png"
    ...
    def analyze_with_gpt4o(self, image_path: str) -> dict:
        ...
        try:
            ...
            # Get analysis prompt
            prompt = self.get_section_2_3_analysis_prompt()
    ...
    def apply_changes_to_document(self, analysis_result: dict) -> bool:
        ...
        try:
            ...
            # Apply Section 2_3 changes
            changes_applied = self.apply_section_2_3_changes(doc, analysis_result)
            
            # Save processed document
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_path = f"{self.test_dir}/section_2_3_processed_{timestamp}.docx"
    ...
    def get_section_2_3_analysis_prompt(self) -> str:
        """
        Analysis prompt for Section 2_3 - SAME RULES AS SECTION 2_1
        """
        return """
    ...
    def find_section_2_3_table_row(self, doc: Document) -> tuple:
        ...
        # Section 2_3 should be different from Section 2_1 content
            ...
            if len(table.rows) >= 3 and len(table.columns) >= 2:  # At least 3 rows for multiple sections
                ...
                for row_idx in range(2, len(table.rows)):  # Start from row 2 (3rd row)
                    ...
                    if len(row.cells) >= 2:
                        ...
                        # Look for Section 2_3 indicators (different from Section 2_1)
                ...
                for item in items_to_delete:
                    if item.get("should_delete", False):
                        ...
                        for para_idx, para in enumerate(paragraphs):
                            ...
                            # Strategy 3: Insurance/protection terms matching
                            insurance_terms = ['insurance', 'life', 'tpd', 'income', 'protection', 'premium', 'cover']
                            if any(term in target_text.lower() for term in insurance_terms) and \
                               any(term in para_text.lower() for term in insurance_terms):
    ...
    def apply_section_2_3_changes(self, doc: Document, analysis_data: dict) -> list:
        """
        Apply Section 2_3 changes - SAME LOGIC AS SECTION 2_1
        """
        changes_applied = []
    ...
    def save_analysis_results(self, analysis_data: dict):
        ...
        try:
            analysis_file = os.path.join(self.test_dir, "section_2_3_analysis.json")
    ...
    def run_analysis_only(self, progress_callback=None):
        ...
        print(f"Starting Section_2_3 Analysis")
        ...
        print(f"📊 Step 1: Loading PDF and cutting Section_2_3...")
        section_image_path = self.load_extracted_image()
        if not section_image_path:
            print(f"❌ Section_2_3 Analysis Failed!")
            return False
        ...
        print(f"🧠 Step 2: Analyzing Section_2_3 with GPT-4o...")
        ...
        print(f"🎉 Section_2_3 Analysis Complete!")
        return True
    
    def run_test(self):
        """Run the complete Section 2_3 test"""
        print(f"Starting Section 2_3 Test (Same rules as Section 2_1)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="2 3 Tester")
    ...
    tester = Section2_3Tester(pdf_path=args.pdf_path)
